...
class Buy extends Core {
	...
	public function getContent() {
		...
		$this->templateEdit("title_content", "Список пакетов");
		if($user['confirm'] == '1') {
			...
			$faq = "";
			$fq = $mysqli->query("SELECT * FROM `packages` ORDER BY `price` ASC");
			if($fq->num_rows > 0) {
				$f = $mysqli->assoc($fq);
				do {
					$includes_value = "";
			...
			if($adds->set("bonuses") == "1") {
				$promocode = R::read("default", "buy/promocode", array());
			}
			...
			$data->change("uri", URI);
			$data->change("promocode", $promocode);

$(document).ready(function() {
	...
	$(".web-change-account").click(function(e) {
		e.preventDefault();
		if(confirm("Вы хотите выйти?")) {
			turnOff();
			localStorage.removeItem("authtoken");
			localStorage.removeItem("device_id");
			$(".web-binomo-auth-wrap").addClass("false");
		}
	});
	...
	$(".web-pair-checked").each(function() {
		settings.percents[$(this).attr("data-binomo")] = Number($(this).attr("data-win"));
	});
	...
	function openDealPlatform(symbol, position, rate) {
		var balanceType = settings.balanceType == ":demo" ? true : false;
		settings.opening[symbol].open = true;
		$.ajax({
			method: "POST",
			url: "../index.php?page=ajax&ajax-handle=binomo_data&time=" + Date.now(),
			data: {
				"action" : "open-deal",
				"symbol" : symbol,
				"position" : position,
				"rate" : rate,
				"minutes" : settings.timeExp,
				"demo" : settings.balanceType
			},
			success: function(data) {
				if(data == "balance") {
					$(".web-message").text("На балансе недостаточно средств").slideDown(300);
					turnOff();
				}
				else if(data == "deal_amount_max") {
					$(".web-message").text("Робот достиг максимальной суммы сделки $1000.00").slideDown(300);
				}
			}
		});
	}
						...
						if(settings.opening[symbol].knees < settings.numberOfKnees) {
							if(!settings.opening[symbol].open) {
								...
								if(typeof zero == "undefined" && !settings.opening[symbol].zero) {
									...
									settings.opening[symbol].opened = false;
								}
	...
	function closeThenProcessed(symbol, uniqid) {
		setTimeout(function() {
			if(!settings.isAfterMartingale && !settings.isUserMartingale) {
				if(typeof settings.opening[symbol] != "undefined") {
					if(settings.opening[symbol].id == uniqid && !settings.opening[symbol].opened) {
						delete settings.opening[symbol];
					}
				}
			}
	...
	$(".web-launch").click(function() {
		if(!settings.isRobotStarted) {
			...
			if(!$(".web-pair-checked:checked").length) {
				...
				settings.balanceType = $(".web-set-balance-type").val() == ":demo" ? ":demo" : "";

				settings.isRobotStarted = true;
				$(".web-message").slideUp(300);
				$("body, html").animate({scrollTop: 0}, 300);
				$(".web-robot-start").addClass("fade-in");
				settings.socket = new WebSocket("wss://ws.binomo.com/ws");
				settings.socket.onopen = function() {
					if(localStorage.getItem("authtoken") == null || localStorage.getItem("device_id") == null) {
						$(".web-binomo-auth-wrap").addClass("false");
						turnOff();
					}
					else {
						settings.isSocketConnected = true;
						settings.socket.send('{"action":"auth","authtoken":"'+localStorage.getItem("authtoken")+'","device":"web","device_id":"'+localStorage.getItem("device_id")+'"}');
						setTimeout(function() {
							if(!settings.isSocketInitData) {
								$(".web-message").text("Невозможно выполнить соединение").slideDown(300);
								turnOff();
							}
						}, 5000);
					}
				...
				settings.socket.onmessage = function(msg) {
					if(settings.isSocketConnected) {
						...
						for(var i = 0; i < data.length; i++) {
							var that = data[i];
							if(that.action == "unauthorized") {
								$(".web-binomo-auth-wrap").addClass("false");
								turnOff();
							}
							else if(that.action == "close_deal_batch") {
								var balance = settings.balanceType == ":demo" ? data[i+1].data.demo_balance / 100 : data[i + 1].data.balance / 100;
								var symbol = that.data.ric;
								var info = settings.opening[symbol];
								var bid = info.bid;
								var saveRate = info.rate;
								var profit = info.rate;
								var status = "zero";
								if(info.bid < that.data.end_rate && info.position == "up" || info.bid > that.data.end_rate && info.position == "down") {
									status = "win";
									profit = info.rate * info.winperc / 100;
									delete settings.opening[symbol];
								}
								else if(info.bid == that.data.end_rate) {
									status = "zero";
									settings.opening[symbol].open = false;
									settings.opening[symbol].zero = true;
									if(!settings.isAfterMartingale)
										openDeal(symbol, info.position, true);
								}
								else {
									status = "lose";
									settings.opening[symbol].open = false;
									settings.opening[symbol].zero = false;
									if(!settings.isAfterMartingale)
										openDeal(symbol, info.position);
								}
								if(!Object.keys(settings.opening).length && !settings.isClosedAllDeals)
									turnOff();
								updateItem(info.winperc, symbol, info.position, saveRate, profit, status, "USD", Date.now());
							}
							else if(that.action == "create_deal" && that.data.status == "open") {
								settings.opening[that.data.ric].opened = true;
								settings.opening[that.data.ric].open = true;
								settings.opening[that.data.ric].bid = that.data.entrie_rate;
								settings.opening[that.data.ric].winperc = that.data.payment_rate;
								var lasttime = (Date.parse(that.data.finished_at) - Date.parse(that.data.created_at)) / 1000;
								createItem(that.data.payment_rate, that.data.ric, that.data.trend == "call" ? "up" : "down", that.data.bet / 100, 0, "signal", "USD", Date.now(), lasttime);
							}
							else if(that.action == "authorized" || that.action == "change_balance") {
								if(that.action == "authorized") {
									settings.socket.send('{"action":"subscribe","seq":null}');
					        var syncPing = setInterval(function() {
					        	if(settings.isRobotStarted) {
					          	settings.socket.send('{"action":"ping"}');
					        	}
					        	else {
					        		clearInterval(syncPing);
					        	}
					        }, 60000);
								}
								var balanceType = settings.balanceType;
								settings.balance = balanceType == ":demo" ? that.data.demo_balance / 100 : that.data.balance / 100;
		...
		$.ajax({
			...
			success: function(data) {
				if(data != "error_request") {
					...
					for(var i = 0; i < data.length; i++) {
						openDeal($(".web-pair-checked[value="+data[i].symbol+"]").attr("data-binomo"), data[i].position);
					}
				}
			},
			fail: function() {
	...
	function getPercents() {
		$.ajax({
			method: "POST",
			url: "../index.php?page=ajax&ajax-handle=binomo_data",
			data: {
				"action" : "percents"
			},
			success: function(data) {
				...
				if(data.code == "success") {
					for(percent in data.percents) {
						settings.percents[percent] = data.percents[percent];
					}
				}
			},
			fail: function() {
	...
	function createItem(percent, symbol, position, rate, profit, status, currency, index, blasttime) {
		if(typeof currency == "undefined")
			var currency = "";
		if(typeof index == "undefined")
			var index = Date.now();
		...
		$template.find(".web-item-img").attr("src", ($template.find(".web-item-img").attr("src").replace("{symbol}", drawData.symbol.replace("/", ""))));
	...
	function initTimer(id, lasttime) {
		$("#web-tick-"+id).find(".web-process-loader").animate({"width" : "0%"}, lasttime * 1000, function() {
			$("#web-tick-"+id).remove();
		});
	}
...
$(document).ready(function() {
	$(".web-set-time").change(function() {
		if($(this).val() == "5") {
			if($(".web-set-martin + a").hasClass("checked")) {
				$(".web-set-martin").click();
				$(".web-set-martin + a").removeClass("checked").addClass("disabled");
			}
		}
		else {
			$(".web-set-martin + a").addClass("checked").removeClass("disabled");
			$(".web-set-martin").click();
		}
	});
	$("body").on("click", ".web-binomo-btn", function() {
		var btn = $(".web-binomo-btn"),
				btnVal = $(btn).html();
		$(btn).html("<i class='fa fa-spin fa-spinner'></i> " + btnVal);
		$.ajax({
			method: "POST",
			...
			data: {
				"action" : "auth",
				"email" : $(".web-binomo-login").val(),
				"password" : $(".web-binomo-password").val()
			},
			success: function(data) {
				$(btn).html(btnVal);
				data = JSON.parse(data);
				if(data.code == "success") {
					...
					$(".web-binomo-auth-wrap").removeClass("false");
					data['authtoken'] = $(".web-binomo-authtoken").val();
					data['device_id'] = $(".web-binomo-device-id").val();
					localStorage.setItem("authtoken", data['authtoken']);
					localStorage.setItem("device_id", data['device_id']);
				}
				else {
					showMessage("Логин или пароль неверны");
				}
			},
			fail: function() {
				$(btn).html(btnVal);
